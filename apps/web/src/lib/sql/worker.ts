...
import { generateSyntheticData } from "../data/datasets";
...
export async function benchmarkAcrossVolumes(
  ...
  let prevRows = -1;
  let prevOriginalMs = -1;
  let prevOptimizedMs = -1;
  ...
  for (const rows of volumes) {
    ...
    // ── Proactive Bailout ──────────────────────────────────────────
    // If the next volume is predicted to take an egregiously long time 
    // (e.g. > 15 seconds) based on empirical scaling, bail out early.
    // This prevents the tab from freezing for minutes if a query just 
    // barely squeaked under the maxMsPerRun budget this round but the 
    // next volume is 5x larger and the query scales quadratically.
    const nextRows = volumes[volumes.indexOf(rows) + 1];
    if (nextRows) {
      const checkBailout = (currentMs: number, prevMs: number) => {
        if (currentMs <= 0) return false;
        if (currentMs > 5 && prevMs > 5 && prevRows > 0) {
          const rawExp = Math.log(currentMs / prevMs) / Math.log(rows / prevRows);
          const exp = Number.isFinite(rawExp) ? Math.min(Math.max(rawExp, 0.5), 3) : 1.2;
          const predictedMs = currentMs * Math.pow(nextRows / rows, exp);
          return predictedMs > maxMsPerRun * 3; // > 12s predicted
        }
        // Fallback: assume worst-case quadratic scaling if we lack data
        const predictedMs = currentMs * Math.pow(nextRows / rows, 2);
        return predictedMs > maxMsPerRun * 5; // > 20s predicted
      };

      if (!originalTooSlow && checkBailout(originalMs, prevOriginalMs)) {
        originalTooSlow = true;
      }
      if (!optimizedTooSlow && checkBailout(optimizedMs, prevOptimizedMs)) {
        optimizedTooSlow = true;
      }
    }

    prevRows = rows;
    prevOriginalMs = originalMs;
    prevOptimizedMs = optimizedMs;
  }
...
self.onmessage = async (e: MessageEvent) => {
  ...
  try {
    ...
    switch (type) {
      case "runQuery":
        result = await runQuery(payload.sql, payload.data);
        break;
      case "explainQueryPlan":
        result = await explainQueryPlan(payload.sql, payload.data);
        break;
      case "explainQueryPlanWithIndexes":
        result = await explainQueryPlanWithIndexes(payload.sql, payload.data, payload.indexDdl);
        break;
      case "verifyIndexImpact":
        result = await verifyIndexImpact(payload.sql, payload.data, payload.indexDdl);
        break;
      case "compareQueries":
        result = await compareQueries(payload.originalSql, payload.optimizedSql, payload.data, payload.indexDdl);
        break;
      case "compareResultSets":
        result = await compareResultSets(payload.originalSql, payload.optimizedSql, payload.data, payload.indexDdl);
        break;
      case "benchmarkAcrossVolumes":
        result = await benchmarkAcrossVolumes((rows) => {
           return generateSyntheticData(payload.data as any, rows);
        }, payload.originalSql, payload.optimizedSql, payload.indexDdl, payload.volumes, payload.options);
        break;

      case "estimateBenchmarkTime":
        result = await estimateBenchmarkTime((rows) => {
           return generateSyntheticData(payload.data as any, rows);
        }, payload.sql, payload.volumes, payload.options);
        break;
      case "estimateSingleRunTime":
        result = await estimateSingleRunTime((rows) => {
           return generateSyntheticData(payload.data as any, rows);
        }, payload.sql, payload.targetRows);
        break;
    }
